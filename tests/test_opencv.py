...
from sushi.backend_opencv import (
    opencv_count_pixels_single,
    opencv_draw_single,
    opencv_draw_single_inplace,
    opencv_drawloss_single,
)
from sushi.utils import np_image_mse


@dataclass
class OpencvTestImageData:
...
@pytest.fixture
def setup_data() -> OpencvTestImageData:
...
def test_opencv_draw_match_reference(setup_data: OpencvTestImageData) -> None:
    """
    ...
    drawn_image_np = opencv_draw_single(image_np, vertices, color)
    ...
    try:
        ...
        failed_image_path = Path(__file__).parent / "data/failed_opencv_draw.png"
...
def test_opencv_count_pixels_single(setup_data: OpencvTestImageData) -> None:
    """
    ...
    counted_pixels = opencv_count_pixels_single((height, width), vertices)

    assert abs(counted_pixels - expected_pixel_count) < 200


def test_opencv_drawloss_single(setup_data: OpencvTestImageData) -> None:
    """
    ...
    draw_loss_without_base = opencv_drawloss_single(
        image_np, target_image_np, vertices, color
    )
    draw_loss_with_base = opencv_drawloss_single(
